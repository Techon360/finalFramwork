...
public class TC_MakeBillWithOtherPaymentModeForFreshPatient_17 extends BaseClass {
...
//    		prp.cbox_isexpired();   //check isexpired
    ...
    @Test(description = "Make Bill with other payment mode", priority = 2)
    public void MakeBillwithOtherMode() throws InterruptedException {
		 ...
		 Thread.sleep(900);
		 bp.clickNoButtonOnsecondPopUp();
		 logger.debug("Clicked on No for create other type of receipt");
		 Thread.sleep(900);
		 logger.info("Bill is created amount is "+billamountreceived);
		 System.out.println("Bill is created amount is "+billamountreceived);
    }
    @Test(description = "create other payment mode receipt", priority=3)
